...
try {
    ...
    $yearsStmt = $pdo->query("SELECT DISTINCT YEAR(published_at) AS y FROM news_items ORDER BY y DESC");
    ...
    if ($selectedYear > 0) {
        $countStmt = $pdo->prepare("SELECT COUNT(*) FROM news_items WHERE YEAR(published_at) = :year");
...
function build_news_url(int $year, int $page): string {
    return '/noticias/index.php?ano=' . urlencode((string)$year) . '&pagina=' . urlencode((string)$page);
}

page_header('Noticias');
?>
<div class="container py-4">
    <h1 class="section-title h3 mb-4">Noticias</h1>

    <?php if (!empty($years)): ?>
        <div class="d-flex flex-wrap gap-2 mb-4">
            <?php foreach ($years as $year): ?>
                <a class="btn btn-sm <?= $year === $selectedYear ? 'btn-primary' : 'btn-outline-primary' ?>" href="<?= e(build_news_url($year, 1)) ?>">
                    <?= e((string)$year) ?>
                </a>
            <?php endforeach; ?>
        </div>
            ...
            <div class="col-md-6 col-xl-4">
                <a class="card card-link news-card h-100" href="/noticias/ver.php?slug=<?= urlencode($item['slug']) ?>">
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <span class="news-cta mt-auto">Ler noticia</span>
                    </div>
                </a>
            </div>
        <?php endforeach; ?>
    ...
    <?php if (empty($items)): ?>
        <div class="alert alert-warning mt-4 mb-0">Nenhuma noticia encontrada para o ano selecionado.</div>
    <?php endif; ?>
